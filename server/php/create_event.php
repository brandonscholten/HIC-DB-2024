...
$insert = $mysqli->prepare("INSERT INTO organizes(user_id, event_id) VALUES ((SELECT user_id FROM session WHERE session_id = ?),(SELECT event_id FROM event WHERE organizer = (SELECT user_id FROM session WHERE id = ?)))");
